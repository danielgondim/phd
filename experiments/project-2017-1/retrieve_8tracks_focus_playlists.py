...
query = "SELECT M.user_id, MT.mix_id, M.name, M.tag_list_cache, MT.track_id, T.name, T.performer FROM 8tracks.mixes AS M, 8tracks.mixes_tracks AS MT, 8tracks.tracks AS T WHERE (M.id = MT.mix_id AND MT.track_id = T.id AND M.tag_list_cache LIKE '%focus%');"
...
while current_row != ():
	...
	while (current_row[0][1] == mix_id):
		...
		ids_map[mix_id].append([user_id, mix_id, mix_name, mix_tags, track_id, track_name, artist_name])
		current_playlist.append([user_id, mix_id, mix_name, mix_tags, track_id, track_name, artist_name])
		current_row = cur.fetchmany()
		if (current_row == ()):
			break
	'''with open('/home/danielgondim/workspace/project-2017-1/playlists_not_focus/' + str(mix_id) + '.csv', 'w') as output:
		writer = csv.writer(output, lineterminator='\n')
		writer.writerows(current_playlist)'''
	playlists_count += 1
	print 'Playlist %d com o ID: %d' % (playlists_count,mix_id)
	if len(ids_map) >= 180:
		break

print len(ids_map)

for playlist in ids_map:
	with open('/home/danielgondim/workspace/project-2017-1/playlist_focus_v2/' + str(playlist) + '.csv', 'w') as output:
		writer = csv.writer(output, lineterminator='\n')
		writer.writerow(header)
		writer.writerows(ids_map[playlist])