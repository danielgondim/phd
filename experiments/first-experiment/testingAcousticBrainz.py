...
for current_user in LASTFM_USERS_DETAILS:
	...
	while (current_date <= final_date):
		if (current_date.weekday() < 5):
			#pegar musicas pela manha
			#print "Musicas da manha do dia: " + str(current_date)
			response = requests.get(api_rest_with_timestamp(current_date, current_user, LASTFM_USERS_DETAILS[current_user][2]))
			data = response.json()
			for track in data['recenttracks']['track']:
				music_found = False
				track_id = ""
			...
			#musicas da tarde
			current_date += datetime.timedelta(hours=LASTFM_USERS_DETAILS[current_user][1])
			#print "Musicas da tarde do dia: " + str(current_date)
			response = requests.get(api_rest_with_timestamp(current_date, current_user, LASTFM_USERS_DETAILS[current_user][2]))
			data = response.json()
			for track in data['recenttracks']['track']:
			...
			#passando para o inicio do expediente do outro dia
			current_date += datetime.timedelta(hours=LASTFM_USERS_DETAILS[current_user][3])
		else:
			#se for fds passa o dia
			#print "Hoje eh fim de semana, nao teve trabalho!!!!"
			#print "################################"
...
with open('spotify_features_data.json', 'w') as outfile:
